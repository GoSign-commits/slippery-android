...
Session start: check if `opencode serve` is running on port 4096 (`curl -s http://localhost:4096/doc`); if not, start it in the background. In the first response, always include: "Watch/intervene live: `opencode attach http://localhost:4096` (paste into a real terminal — exact `--session <id>` given if work gets delegated)".
